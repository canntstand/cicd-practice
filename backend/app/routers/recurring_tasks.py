...
async def complete_uncomplete_recur_task(
    ...
    resp = await rt.complete_uncomplete_recur_task(user_task_id, user_id, db)
...
async def get_recur_task(
    ...
    task = await rt.get_recur_task(user_id, user_task_id, db)
...
async def update_recur_task(
    ...
    resp = await rt.update_recur_task(user_task_id, body, db)
...
async def delete_recur_task(
    user_task_id: int,
    db: AsyncSession = Depends(get_db),
    user_id: int = Depends(get_current_user),
):
    resp = await rt.delete_recur_task(user_id, user_task_id, db)
